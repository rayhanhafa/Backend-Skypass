...
const createFlight = async (req, res, next) => {
  ...
  try {
    ...
    const departingAirport = await Airport.findOne({
      where: { iata_code: departure_iata_code },
    });
    const arrivingAirport = await Airport.findOne({
      where: { iata_code: arrival_iata_code },
    });
...
const getAllFlights = async (req, res, next) => {
  try {
    ...
    // Hindari error jika seat_class tidak dikirim
    const seatClassLower = seat_class ? seat_class.toLowerCase() : null;
    ...
    if (departure_airport)
      sqlQuery += ` AND f.departure_airport = '${departure_airport}'`;
    if (departure_date)
      sqlQuery += ` AND f.departure_date = '${departure_date}'`;
    if (arrival_airport)
      sqlQuery += ` AND f.arrival_airport = '${arrival_airport}'`;
    ...
    if (seatClassLower) {
      sqlQuery += ` AND p.seat_class = '${seatClassLower}'`;
    }
    ...
    const flights = await sequelize.query(sqlQuery, {
      type: QueryTypes.SELECT,
    });
    flights.forEach((flight) => {
      flight.seat_class = flight.seat_class
        .split(" ")
...
const getFlightById = async (req, res, next) => {
  try {
    ...
    if (!flight) {
      return next(
        new ApiError(`Flight with ID: ${req.params.id} not found`, 404),
      );
    }
...
const deleteFlight = async (req, res, next) => {
  try {
    ...
    if (!flight) {
      next(
        new ApiError(`Flight with this ID : ${req.params.id} not found`, 404),
      );
    }
    ...
    let favoriteDestination = [
      {
        ...
        to: "Singapore", //city
        continent: "Asia",
        departureDate: "2024-07-24", //tanggal kedatangan
        returnDate: "2024-07-29", //tanggal kepulangan
        passengersTotal: 3,
        passengersAdult: 3,
        passengersBaby: 0,
        passengersChild: 0,
        ...
        price: 999999,
        imageUrl:
          "https://smansasingaraja.sch.id/wp-content/uploads/2021/09/singapore-1-960x600-1.jpg",
        airline: "Lion air",
      },
      {
        isRoundTrip: true, //set aja true
        ...
        to: "Medina", //city
        continent: "Asia",
        departureDate: "2024-07-25", //tanggal kedatangan
        returnDate: "2024-07-30", //tanggal kepulangan
        passengersTotal: 3,
        passengersAdult: 3,
        passengersBaby: 0,
        passengersChild: 0,
        ...
        price: 1500000,
        imageUrl:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/c/cd/Madeena_masjid_nabavi_12122008230.jpg/800px-Madeena_masjid_nabavi_12122008230.jpg",
        airline: "Saudia",
      },
      {
        isRoundTrip: true, //set aja true
        ...
        continent: "Europe",
        departureDate: "2024-07-24", //tanggal kedatangan
        returnDate: "2024-07-27", //tanggal kepulangan
        passengersTotal: 3,
        passengersAdult: 3,
        passengersBaby: 0,
        passengersChild: 0,
        ...
        price: 1300000,
        imageUrl:
          "https://www.oliverstravels.com/blog/wp-content/uploads/2019/05/Sintra-Coast-V2.jpg",
      ...
      },
      {
        ...
        to: "Tokyo", //city
        continent: "Asia",
        departureDate: "2024-07-28", //tanggal kedatangan
        returnDate: "2024-08-02", //tanggal kepulangan
        passengersTotal: 3,
        passengersAdult: 3,
        passengersBaby: 0,
        passengersChild: 0,
        ...
        price: 1400000,
        imageUrl:
          "https://www.planetware.com/wpimages/2020/06/japan-tokyo-to-mt-fuji-best-ways-to-get-there-by-group-tour.jpg",
      ...
      },
      {
        ...
        to: "Los angeles", //city
        continent: "America",
        departureDate: "2024-07-26", //tanggal kedatangan
        returnDate: "2024-08-01", //tanggal kepulangan
        passengersTotal: 3,
        passengersAdult: 3,
        passengersBaby: 0,
        passengersChild: 0,
        ...
        price: 2000000,
        imageUrl:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/7/74/LA_Downtown_View_%28cropped%29.jpg/800px-LA_Downtown_View_%28cropped%29.jpg",
...
const validateQueryParams = (params) => {
  const { page, limit, seats_available, departure_date, arrival_date } = params;

  if (page && isNaN(parseInt(page)))
    throw new ApiError("Invalid page number", 400);
  if (limit && isNaN(parseInt(limit)))
    throw new ApiError("Invalid limit number", 400);
  if (seats_available && isNaN(parseInt(seats_available)))
    throw new ApiError("Invalid seats available", 400);
